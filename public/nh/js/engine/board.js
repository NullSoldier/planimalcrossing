...
Board.prototype.loadLayout = function loadLayout (layout) {
    ...
    this.background = this.R.image(Board.toFullPath('img/layouts/demolayout.png'), 0, 0, 0, 0);
...
function openMapBuilder(evt) {
    var mapMode = document.getElementsByClassName("map-mode");
    if (mapMode[0].style.display == "none") {
        mapMode[0].style.display = "block";
    }
    else {
        mapMode[0].style.display = "none";
    }
}
...
Board.prototype.openMapSelector = function (x, y) {
    ...
    maps.forEach(function (file) {
        ...
        if (add) {
            ...
            img.setAttribute("src", `/nh/img/layouts/maps/${file}`);
...
Board.prototype.drawTile = function drawTile (location, tile, replace) {
    ...
    var isMapEditing = tile ? tile.includes('maptile') : false;
    ...
    if (tile === 'map') {
        //Disable map setter for now on new horizons
    ...
    if (this.brush.mapErase) {
        if (this.mapTiles[hardY][hardX] && this.mapTiles[hardY][hardX].length > 0) {
            this.mapTiles[hardY][hardX].shift().remove()
            if (this.mapTiles[hardY][hardX].length > 0) {
                this.mapTiles[hardY][hardX].shift().remove()
            }
        }
        return;
    ...
    if (isMapEditing) {
        ...
        if (this.mapTiles[hardY][hardX]) {
            if (this.mapTiles[hardY][hardX].length > 1) {
                this.mapTiles[hardY][hardX].shift().remove()
            }
            if (tile.includes("full") && this.mapTiles[hardY][hardX].length > 0) {
                this.mapTiles[hardY][hardX].shift().remove()
            }
        ...
        if (tile) {
            ...
            var opacity = 1;
            if (tile.includes("tier-2")) {
                opacity = 0.6
            }
            if (tile.includes("tier-1")) {
                opacity = 0.35
            }
            ...
            if (!this.mapTiles[hardY][hardX]) {
                this.mapTiles[hardY][hardX] = [];
            }

            this.mapTiles[hardY][hardX].push(newTile);

            if (this.tiles[hardY][hardX]) {
                var objectType = this.tiles[hardY][hardX].attr('tileType');
                this.tiles[hardY][hardX].remove();
                var newObj = this.R.use(objectType);
                newObj.attr({
                    x: location.x,
                    y: location.y,
                    tileType: objectType,
...
Board.prototype.preDrawSprites = function preDrawSprites() {
    data.tiles.forEach(function (tile) {
        var tileImage = this.R.image(Board.toFullPath('img/flat-tiles/'+ tile +'.png'), 0, 0, this.tileSize, this.tileSize);
        tileImage.attr({
            id: tile
        });
    ...
    Object.entries(data).forEach(entry => {
        ...
        let tiles = entry[1];
        if(category !== 'buildings' && category !== 'tiles' && category !== 'structures') {
    ...
    data.buildings = $.extend({}, data.buildings, data.structures)
...
Board.prototype.exportData = function exportData() {
    ...
    this.mapTiles.forEach(function (yTiles) {
        yTiles.forEach(function (xtiles) {
            if (xtiles) {
                xtiles.forEach(function (tile) {
                var tileData = {
                    type: tile.attr('tileType'),
                    y: tile.attr('y'),
                    x: tile.attr('x')
...
Board.prototype.importData = function importData(data, cb) {
    ...
    // import tiles
    farmData.mapTiles.forEach(function (tile) {
...
Board.toFullPath = function toFullPath(relativePath) {
    return window.location.origin + '/nh/'+ relativePath;
};
