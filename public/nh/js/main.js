...
$().ready(function () {
    var board = new Board('#editor', 1792, 1536,'img/layouts/demolayout.png');
    ...
    var planId = window.location.pathname.match(/\/nh\/(.*)\//);
    ...
    function loadLayout (layout) {
        ...
        $('#editor').html('');
        board = new Board('#editor', 1792, 1536, "layout");
        $('#editor,.editor').css({
            width: 1792,
            height: 1536
        });
    ...
    $('#save,.render-farm').click(function (e) {
        ...
        $.ajax({
            url: '/api/'+ (isRender ? 'render' : 'save-nh'),
            ...
            if (isRender) {
                ...
                if (data.id) {
                    window.location.href = '/nh/' + data.id;
                }
            }
        });
    });
